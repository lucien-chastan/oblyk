                ...
                <table>
                    ...
                    <tbody>
                        @foreach($topics as $topic)
                            <tr>
                                <td><a class="text-bold" href="{{ route('topicPage',['topic_id'=>$topic->id,'topic_label'=>str_slug($topic->label)]) }}">{{$topic->label}}</a></td>
                                <td class="text-center">{{$topic->category_id}}</td>
                                <td class="text-center">{{$topic->views}}</td>
                                <td class="text-center">{{$topic->nb_post}}</td>
                                <td class="grey-text text-center">{{$topic->created_at->format('d M Y à H:i')}}</td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
